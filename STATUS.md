...
- **brazil-fiscal-policy-through-end-of-2027** (active)
- **semiconductor-supply-chain** (active)
...
3ccb918 #7: Regression guard installed — test_no_gate_bypass.py, pre-commit hook. Zero bypasses across 3,397 files.
54ebf09 #6: topic_morning_brief.py (generalizes Mexico brief). Residue audit surfaced 23 Mexico-specific files outside archive — surfaced for authorization.
3a9423b #5 cont'd: GATE_EXEMPT for remaining low-priority files (har_analyzer, query_map, mexico_brief migration residue)
5c5e4a7 #5: Low-priority bypasses — Pattern A (translate_es_to_en, har_analyzer, etc.) route through gate. Pattern B (render_audio, benchmark_compare) marked GATE_EXEMPT
588ef32 #4: Medium-priority bypass fixes — priority_triage.py + trade_engine.py route through gate
566ed32 #3: scope_check Option B — routes through scope_classifier task type in llm_gate (V4 Flash, no escalation, minimal logging)
