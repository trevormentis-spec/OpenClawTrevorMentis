...
def self_test() -> list[str]:
    ...
    return failures
def _get_token() -> str:
    """Get Mapbox token from _env (not .env directly)."""
    from _present_env import get_key as _gk
    token = _gk("MAPBOX_TOKEN")
    if not token:
